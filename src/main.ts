import { videoStart, createCameraBox, scanBarcode, showResult, toggleElements } from "./barcode";


const videoCaptureEl  = document.getElementById("video-capture") as HTMLVideoElement;
...
(async function(){
  ...
  // 未検出だった場合ループする。
  async function loop_scan(element: HTMLVideoElement, formats: any, intarval: number, resultElement: HTMLElement) {
    const barcode = await scanBarcode(element, formats);
    if (!Array.isArray(barcode) || Array.isArray(barcode) && barcode.length == 0) {
      setTimeout(() => loop_scan(videoCaptureEl, barcodeFormats, intarval, resultEl), intarval);
    } else {
